...
class Game {
  ...
  handleKeyPress(evt) {
    ...
    if (evt.key === "ArrowUp") {
      ...
      this.gameCursor.y = this.gameCursor.y + 1 > this.ROWS - 1 ? this.ROWS - 1 : this.gameCursor.y + 1;
    } else if (evt.key === "ArrowLeft") {
      this.gameCursor.x = this.gameCursor.x - 1 < 0 ? 0 : this.gameCursor.x - 1;
    } else if (evt.key === "ArrowRight") {
      this.gameCursor.x =
        this.gameCursor.x + 1 > this.COLUMNS - 1 ? this.COLUMNS - 1 : this.gameCursor.x + 1;
    }
  ...
  handleSaveLevel() {
    ...
    if (filename) {
      ...
      element.setAttribute('href', 'data:application/json; charset=utf-8,' + JSON.stringify(json));
  ...
  drawAddedTiles() {
    this.tiles.forEach(tile => {
      ...
      if (hex) {
        ...
        this.graphics?.drawImage(tile.sprite, point.x, point.y);
      }
    })
  }
  drawGrid() {
    if (this.canvas && this.graphics) {
      ...
      this.grid.forEach((hex) => {
        if (this.graphics && this.canvas) {
          ...
          const isCursorOrNeighbour = isInList(hex, [
            this.gameCursor,
            ...this.getNeighbours()]
          );
          ...
          if (isCursor) {
            this.graphics.fillStyle = 'purple';
            this.graphics.fill();
          } else if (!isCursorOrNeighbour && this.fogOfWar) {
            this.graphics.fillStyle = 'black';
            this.graphics.fill();
          } else {
            this.graphics.strokeStyle = 'grey';
